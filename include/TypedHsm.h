...
// The wrap metafunction accepts a tuple of types and a template template parameter (Wrapper)
...
struct append_unique<T, std::tuple<Ts...>> {
    // Add T to the tuple if the condition is true and T is not found among Ts..., otherwise, leave as is.
...
template <typename From, typename Event, typename To, typename Action = decltype(dummy_action), typename Guard = decltype(dummy_guard)>
struct Transition {
    using from = From;
    using event = Event;
    using to = To;
...
// Base case: No more transitions to process, just return the accumulated states.
template <typename Tuple>
...
struct get_states_impl<Tuple, First, Rest...> {
    ...
    using from_states = typename append_unique<typename First::from, Tuple>::type;
    // Accumulate 'to' state
    using to_states = typename append_unique<typename First::to, from_states>::type;
...
// Forward declaration for transition_map_helper with a default index parameter starting from 0
template <typename From, typename Event, typename TransitionsTuple, size_t Index = 0, typename = void>
...
template <typename From, typename Event, typename To, typename Action, typename Guard>
struct is_transition_match<Transition<From, Event, To, Action, Guard>, From, Event> : std::true_type {};

// Iteration through the transitions
template <typename From, typename Event, typename... Transitions, size_t Index>
struct transition_map_helper<From, Event, std::tuple<Transitions...>, Index,
    std::enable_if_t<(Index < sizeof...(Transitions))>> {
    using CurrentTransition = std::tuple_element_t<Index, std::tuple<Transitions...>>;
    static constexpr bool match = is_transition_match<CurrentTransition, From, Event>::value;

    using type = typename std::conditional_t<match,
                                             CurrentTransition,
                                             typename transition_map_helper<From, Event, std::tuple<Transitions...>, Index + 1>::type>;
};
...
inline constexpr bool has_valid_transition_v = !std::is_same_v<typename TransitionMap<State, Event, Transitions>::type, void>;
...
template<typename T>
struct has_exit<T, std::void_t<decltype(std::declval<T>().exit())>> : std::true_type {};
...
template<typename T>
struct has_entry<T, std::void_t<decltype(std::declval<T>().entry())>> : std::true_type {};
...
template<typename T>
inline constexpr bool is_state_trait_v = std::conjunction_v<has_transitions<T>, std::negation<is_hsm_trait<T>>>;
...
struct Hsm : T {
    ...
    using initial_state = typename std::decay_t<decltype(std::get<0>(transitions{}))>::from;
    ...
    bool handle(Event e = Event()) {
        return std::visit([this, e](auto* state) {
            using State = std::decay_t<decltype(*state)>;
    ...
    void handleEventForState(State* state) {
        using transition = typename TransitionMap<State, Event, transitions>::type; // Assume TransitionMap provides the matching transition
        // Optional Guard
...
struct wrap_transition {
    ...
    using wrap_from = std::conditional_t<is_state_trait_v<from>, convert_to_hsm_t<from>, from>;
    using wrap_to = std::conditional_t<is_state_trait_v<to>, convert_to_hsm_t<to>, to>;
...
struct wrap_transitions<std::tuple<Ts...>> {
    using type = decltype(std::tuple_cat(std::tuple<wrap_transition_t<Ts>>{}...));
};

template<typename... Ts>
using wrap_transitions_t = typename wrap_transitions<Ts...>::type;

// Define the wrapper for state traits to convert them into HSMs including their transitions
template<typename T>
struct convert_to_hsm <T, std::enable_if_t<is_state_trait_v<T>>> {
    struct Traits : T {
